...
public class IntegrationFixture
{
    ...
    public void SerializesData(int messageCount)
    {
        ...
        var reader = new BuildEventArgsReader(binaryReader, GetBinaryLoggerFileFormatVersion());
        var eventArgs = new List<BuildEventArgs>();
        ...
        var c = 0;
        foreach (var eventArg in eventArgs.Skip(1))
        {
    ...
    public void NamedPipeSupportsCancellation()
    {
        ...
        {
            using (var server = new NamedPipeLoggerServer("Foo", tokenSource.Token))
            {
                // When
    ...
    public void SendsDataOverNamedPipe(int messageCount)
    {
        ...
        var eventArgs = new List<BuildEventArgs>();
        int exitCode;
        using (var server = new NamedPipeLoggerServer("foo"))
        {
            server.AnyEventRaised += (s, e) => eventArgs.Add(e);
    ...
    private int RunClientProcess(IPipeLoggerServer server, string arguments, int messages)
    {
        ...
        try
        {
            var testDirectory = Path.GetDirectoryName(typeof(IntegrationFixture).Assembly.Location)
